...
sequenceDiagram
    ...
    Message Queue->>Promotion Service: StockDeductedEvent
    Promotion Service->>Promotion Service: 쿠폰 사용 처리
    Promotion Service->>Message Queue: CouponAppliedEvent
    ...
    alt 포인트 부족
        ...
        Message Queue->>Promotion Service: 쿠폰 복원
...
### 5.1 Eventually Consistent 설계

- **Immediate Consistency 불필요 영역**: 상품 랭킹, 통계 데이터
- **Strong Consistency 필요 영역**: 재고, 포인트 잔액

### 5.2 CQRS (Command Query Responsibility Segregation) 적용
...
**Phase 1: 읽기 전용 서비스 분리 (2개월)**
- Product Service의 조회 API 분리
- 기존 모놀리스와 병행 운영

**Phase 2: 독립적 도메인 분리 (3개월)**
- User Service 분리
- Promotion Service 분리

**Phase 3: 핵심 트랜잭션 분리 (4개월)**
- Payment Service 분리
- Order Service 분리
- Saga 패턴 구현
