...
public class ProductRankRedisRepositoryImpl implements ProductRankRedisRepository {
    ...
    @Override
    public Set<TypedTuple<Object>> getTopProducts(String date, long count) {
        String key = PRODUCT_RANK_KEY_PREFIX + date;
    ...
    public List<DomainProductRank> getTopProductsAcrossDays(List<String> dates, long count) {
        ...
        try {
            ...
            redisTemplate.opsForZSet().unionAndStore(keys[0], Arrays.toString(Arrays.copyOfRange(keys, 1, keys.length)), destinationKey);
            
            // 상위 랭킹 조회
            Set<TypedTuple<Object>> topProducts = redisTemplate.opsForZSet().reverseRangeWithScores(destinationKey, 0, count - 1);
            ...
            for (TypedTuple<Object> tuple : topProducts) {
